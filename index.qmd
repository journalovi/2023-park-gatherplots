...
<!-- Teaser -->
::: {#fig-teaser}
![](figures/teaser.svg){fig.alt="Gatherplots showing a dataset related to cars, yielding overplotting in normal scatterplots."}

**Cars dataset.**
Gatherplots showing a dataset related to cars, yielding overplotting in normal scatterplots.
The gatherplot in (a) shows Cylinders (categorical) vs. MPG (continuous), highlighting the overall distribution of MPG values of cars with different cylinders.
...
The gatherplot in (b) shows `Cylinders` (categorical) vs. `Origin` (categorical), partitioning the graphical axes into intervals and packing points into groups for each interval. 
In (c), both X-axis and Y-axis show the same continuous variable (MPG).
...
::: {#fig-aspect-ratio}
![](figures/aspect-ratio.svg){fig.alt="Main layout modes for gatherplots."}

**Main layout modes for gatherplots.**
(a) absolute mode with constant aspect ratio, which maintains the aspect ratio; (b) normalized mode of (a).
The rate of male survivors in each passenger class is not easy to compare.
(c) streamgraph mode, where each cluster maintains the number of element in the shorter edge, making it easier to see the distribution of the subgroups along the Y axis.
:::
...
<!-- 
% \subsection{Aspect Ratio and Alignment}

% Maintaining the aspect ratio of all stacked groups means that the size
% of the group is represented by its area, not length.
% According to Cleveland and McGill~\cite{Cleveland1985}, length is more
% effective than the area for graphical perception.
% Figure~\ref{fig:lengthBasedLayout} shows the problems associated with
% the gather layout for size comparison.
% In this view, the items are stacked along the vertical axis to make
% the size comparison along the horizontal axis easier.
% The stacked group heights are fixed to be equal so that the width
% represent the size of each group.
% However, the groups have drastically different shapes, making
% comparison difficult.
% Furthermore, to make length-based comparison easier, the stacking
% should be aligned to one side of the available space: left, right,
% top, or bottom.

% In this case, the bottom is selected to make it easier to compare
% along the X axis.
% However, this creates two additional problems.
% The first problem is that the center of mass of each stacked group is
% different, so that the concept of belonging to the same value can be
% misleading.
% The second problem is that choosing alignment direction is arbitrary
% and depends on the task.
% For example, in this view it is more difficult to compare along the Y axis.
% In this sense, this layout is biased to the X axis, while sacrificing
% the performance along the Y axis.
% For this reason, the most general choice is to use center alignment
% with aspect ratio based on the assigned range to avoid bias.
 
% \begin{figure}[htb]
%   \centering
%   \includegraphics[width=\columnwidth]{figures/ClassVsSex}
%   \caption{Stacked group layout that supports comparing group sizes
%     because the height of stacked groups is fixed so that width yields
%     size.}
%   \label{fig:lengthBasedLayout}
% \end{figure}
-->
...
::: {#fig-optimal-bin}
![](figures/cont-size.svg){fig.alt="Choosing optimal bin size based on available display space."}

**Choosing optimal bin size based on available display space.**
In (a), there is enough space so that the dot size can be maximized, improving spatial accuracy.
In comparison, in (b) the assigned space is small, so the dot size is determined so that the most crowded bin interval will fit within the assigned space.
...
::: {#fig-cont-cont}
![](figures/cont-cont.svg){fig.alt="Using gatherplots to manage overplotting."}

**Using gatherplots to manage overplotting.**
(a) shows a scatterplot with 5,000 random numbers with severe overplotting in the center area.
In (b), gathering is applied to create a more organized view.
However, the gathering resizes the items so small that it becomes difficult to detect outliers.
(c) shows normalized mode, where the outliers are enlarged.
...
::: {#fig-matrix}
![](figures/undefined.svg){fig.alt="Gatherplot showing survivors of the *Titanic*."}

**Gatherplot showing survivors of the *Titanic*.**
(a) All people on board.
Note that the X and Y axis are not defined.
(b) Color coding for survivors.
(c) Distribution of survivors over class variables.
Here the Y axis is undefined.
(d) Distribution of survivors over the gender.
...
::: {#fig-tick-mark}
![](figures/tick-marks.svg){fig.alt="Various tick mark types."}

**Various tick mark types.**
The blue dotted region represents the area between adjacent tick
marks.
(a) is a typical line type tick mark for scatterplots.
(b) lacks guidelines, which will make anchoring easier.
(c) creates a packed region between adjacent marks. 
(d) uses less clutter, but (e) is the least cluttered.
(f) is the final recommendation, with the data label in the orange
region.
:::
...
::: {#fig-axis-folding}
![](figures/axis-folding.svg){fig.alt="Survivors of the *Titanic* using gatherplots."}
...
Each of the 2,201 survivors had four dimensions, which were all categorical variables: `class` (4 levels), `sex` (2 levels), `port of entry` (4 levels), `survival status` (2 levels).
...
::: {#fig-results}
![Accuracy](plots/vis-accuracy.svg){fig.alt="Accuracy."}
![Time](plots/vis-time.svg){fig.alt="Completion time."}
![Confidence](plots/vis-confidence.svg){fig.alt="Confidence."}
...
::: {#fig-results-detail}
![Accuracy](plots/visxtask-accuracy.svg){fig.alt="Accuracy."}
![Time](plots/visxtask-time.svg){fig.alt="Completion time."}
![Confidence](plots/visxtask-confidence.svg){fig.alt="Confidence."}
...
::: {#fig-correctness}
![](plots/task-performance.png){fig.alt="Correctness results from our crowdsourced user study."}

**Correctness results from our crowdsourced user study.**
(a) Correctness for retrieving value.
(b) Correctness for absolute comparison and ranking.
(c) Correctness for relative/normalized comparison and ranking.}
:::
...
::: {#fig-gatherlens}
![](figures/gatherlens.png){fig.alt="Three examples of a GatherLens in a scatterplot of airline performance data containing 3,000 points."}
